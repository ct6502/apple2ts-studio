...
export class AssemblerService {
    ...
    constructor() {
        this.outputChannel = vscode.window.createOutputChannel('Apple2TS Assembler');
    }

    public async assembleFile(sourceUri: vscode.Uri): Promise<Uint8Array> {
        ...
        try {
            ...
            const assemblerArgs = config.get<string>('assembler.args', '-a -b -C --m65c02');
    ...
    private async assembleWith64tass(sourcePath: string, binaryPath: string, assemblerPath: string, assemblerArgs: string): Promise<Uint8Array> {
    ...
    private async assembleWithSimpleAssembler(sourcePath: string): Promise<Uint8Array> {
